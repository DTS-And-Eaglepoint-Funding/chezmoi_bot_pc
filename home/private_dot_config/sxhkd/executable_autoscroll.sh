...
enableclickscroll=0
clickdelay=0.3 #The grace period for you to release the middle mouse button to activate clickscrolling before it assumes you're holding it down (in seconds), it is inadvisable to set this setting much higher than this.
screen_height=$(xrandr | grep '*' | awk '{ print $1 }' | head -n 1 | cut -d 'x' -f2)
paddingtop=0
paddingbot=0
...
for i in $(seq 1 $devcount); do
    ...
    if [ "$buffer" = "$middlemousebutton" ]; then
        break
    fi
done
screenheight=$(( $screen_height - $paddingbot ))

if [ "$(pidof -x $filename | wc -w)" -gt "2" ]; then
...
while [ "$toggle" = "$middlemousebutton" ]; do
    eval $(xdotool getmouselocation --shell)
    curry=$Y
    currx=$X
    if [ $curry -lt $paddingtop ]; then #Prevent accidental browsertab scroll
        xinput set-prop $mouseid "Coordinate Transformation Matrix" 0 0 0 0 0 0 0 0 100000
        xdotool mousemove_relative 0 $paddingtop
    elif [ $curry -gt $screenheight ]; then #Prevent accidental taskbar scroll
        xinput set-prop $mouseid "Coordinate Transformation Matrix" 0 0 0 0 0 0 0 0 100000
        xdotool mousemove_relative 0 -$paddingbot
    fi
    if [ $enableverticalscroll -eq 1 ]; then
        if [ $curry -gt $starty ]
        then
    ...
    if [ $enablehorizontalscroll -eq 1 ]; then
    ...
    if [ $enableclickscroll -eq 1 ] && [ -z $(xinput --list "Virtual core pointer" | grep -i -m 1 "Button state:" | grep -o "[$middlemousebutton]\+") ]; then
        if [ $(xinput --query-state $mouseid | grep 'button\[' | sort | grep -c down) -gt 0 ]; then #Enables mouse buttons other than middle mouse to stop clickscrolls.
            xinput set-prop $mouseid "Coordinate Transformation Matrix" 1 0 0 0 1 0 0 0 1
            exit
        fi
    else
